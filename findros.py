...
async def test_command(username, machine, command):
    ...
    try:
        exit_code = await asyncio.wait_for(proc.wait(), 3)
        if exit_code == 0:
            return (machine, True)
...
async def test_machines(*, username, command):
    futures = [test_command(username, machine, command) for machine in get_machines(username)]
    print("Checking hosts...", file=sys.stderr)
    for future in asyncio.as_completed(futures):
        yield await future

async def search(*, username, command):
    async for machine, status in test_machines(username=username, command=command):
        if not status:
            continue
        print(machine)

async def connect(*, username, command):
    async for machine, status in test_machines(username=username, command=command):
        if not status:
            continue
...
if __name__ == "__main__":
    ...
    try:
        if ns.search:
            ...
            loop.run_until_complete(search(username=ns.username, command=command))
        else:   
            loop = asyncio.get_event_loop()
            loop.run_until_complete(connect(username=ns.username, command=command))
